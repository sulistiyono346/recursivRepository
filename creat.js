...
Ugly numbers adalah angka yang jika dibagi dengan faktor
prima angka 2, 3 atau 5 menghasilkan angka 1.
...
function getUglyNumber(num) {
  ...
  for (let i = 2; i <= num; i++) {
    var hasil = i
    for (let j = 0; j < pembagi.length; j++) {
      while (hasil % pembagi[j] === 0) {
        hasil = hasil / pembagi[j]
      }
    }
    if (hasil === 1) (
      result.push(i)
